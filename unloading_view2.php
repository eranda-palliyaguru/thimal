...
?>
  ...
  <!-- Content Wrapper. Contains page content -->
	...
	<br>
	<br>
	<a href="unload_load_print.php?d=<?php echo $_GET['d'];?>&lorry_no=<?php echo $_GET['lorry_no'];?>"><button  class="btn btn-info" style="width: 123px; height:35px; margin-top:-8px;margin-left:8px;" >
  Print
 </button></a>
   ...
   $result = $db->prepare("SELECT * FROM loading WHERE  lorry_no='$d2' AND action='load'  ORDER by transaction_id DESC LIMIT 0,1");
	...
	<table id="example1" class="table table-bordered table-striped">
		  ...
		  <?php
		  $result = $db->prepare("SELECT * FROM loading WHERE    date = '$hh' AND lorry_no='$d2' AND term='$term1'  ORDER by transaction_id ASC");
				...
				$lorry=$row['lorry_no'];
				$yard=$row['load_yard'];
				if($yard=='Kaluthara Yard'){
					$yard1=1;
				}
				if($yard=='Payagala Yard'){
					$yard1=2;
				}
			?>	<tr>
				...
				<a  href="unload_loading2.php?root=<?php echo $root;?>&lorry=<?php echo $lorry;?>&rep=<?php echo $rep;?>&yard=<?php echo $yard1;?>"><button  class=" btn btn-info" style="width: 123px; height:35px; margin-:-8px;margin-left:8px;" >
  Edit Load
 </button></a>
          <!-- /.box -->
