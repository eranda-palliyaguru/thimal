          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
				...
				<input type="hidden" value='<?php echo $res=$_GET['load']; ?>'  name="lorry"  >
				
				
				
                <select class="form-control select2" name="products" style="width: 100%;" autofocus >
                  
                  
				  <?php
                $result = $db->prepare("SELECT * FROM loading WHERE lorry_no='$res' AND action='load' ");
		$result->bindParam(':userid', $res);
		$result->execute();
		for($i=0; $row = $result->fetch(); $i++){
	?>
		<option value="<?php echo $row['transaction_id'];?>"><?php echo $row['product_name']; ?>    </option>
	<?php
				}
			?>
                </select>
