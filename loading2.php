...
<?php $id=$_GET['id'];
 $result = $db->prepare("SELECT * FROM loading WHERE transaction_id='$id'  ");
	 ...
	 $result = $db->prepare("SELECT * FROM loading WHERE lorry_id='$lorry_id' ORDER by transaction_id DESC limit 1,1 ");
		$result->bindParam(':userid', $res);
		$result->execute();
		for($i=0; $row = $result->fetch(); $i++){
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Load</li>
              ...
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
				<?php
        $result = $db->prepare("SELECT * FROM loading_list WHERE loading_id='$old_id' and product_code >=9 ");
		$result->bindParam(':userid', $res);
		$result->execute();
		for($i=0; $row = $result->fetch(); $i++){
	?>
