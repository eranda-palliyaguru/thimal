     ...
     <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
				...
				$cash_tot=0;
                //$date=$_GET['id'];
                $result = $db->prepare("SELECT * FROM loading  ");
				$result->bindParam(':userid', $date);
                $result->execute();
                for($i=0; $row = $result->fetch(); $i++){
					$lo_id=$row['transaction_id'];
					
				$result11 = $db->prepare("SELECT * FROM products  ");
				$result11->bindParam(':userid', $date);
                $result11->execute();
                for($i1=0; $row11 = $result11->fetch(); $i1++){	
					$pro_id=$row11['product_id'];
					
				$result1 = $db->prepare("SELECT * FROM stock_log WHERE source_id='$lo_id' and product_id='$pro_id' and action='1' ORDER by id DESC limit 2,1 ");
